...
#endif
{
    ...
    while(PinID < PortNumberOfPortPins)
    {
        ...
        {
            PinID = PinID + 1;
            continue; /* Violation of Misra Rules, Reason : Protection as these pins can lock the MicroController */
        }
        else
        {
        ...
        /*to decide the direction of pin input or output*/
        ...
        default:
         ...
         /* setup digital and analog registers */
...
void Port_SetPinMode( Port_PinType Pin, Port_PinModeType Mode )
{
...
#endif
    {
        ...
        /* setup digital and analog registers */
               switch(PinConfigArray[Pin].DEN_NAMSEL)
               {
               case ENABLE_DIGITAL:
                   SET_BIT( ACCESS_REG_OFFSET(Port_BasePtr , PORT_DIGITAL_ENABLE_REG_OFFSET ) , PinConfigArray[Pin].Pin_Num );   /* Enable Digital MODE */
                   break;
               case ENABLE_ANALOG:
                   SET_BIT( ACCESS_REG_OFFSET(Port_BasePtr , PORT_ANALOG_MODE_SEL_REG_OFFSET ) , PinConfigArray[Pin].Pin_Num );  /* Enable ANALOG MODE */
                   break;
               default:
                   break;
               }
